...
export class ApiRecorder {
  private page: Page;
  private options: Required<ApiRecorderOptions>;
  private recordings: RecordedApiCall[] = [];
  private isRecording = false;
  private requestMap = new Map<string, { startTime: number; request: Request }>();
  ...
  private async handleResponse(response: Response): Promise<void> {
    ...
    const recording: RecordedApiCall = {
      id: requestId,
   ...
   */
  private parseRequestBody(data: string, headers: Record<string, string>): any {
   ...
   */
  private parseResponseBody(buffer: Buffer, headers: Record<string, string>): any {
    const contentType = headers['content-type'] || '';
  ...
  getStatistics(): RecordingStatistics {
    const stats: RecordingStatistics = {
   ...
   * ```
   */
  async saveTestCode(filename: string, options: TestCodeOptions = {}): Promise<string> {

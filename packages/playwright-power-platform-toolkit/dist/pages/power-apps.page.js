...
exports.PowerAppsPage = void 0;
const test_1 = require("@playwright/test");
const power_apps_locators_1 = require("../locators/power-apps.locators");
const canvas_app_page_1 = require("./canvas-app.page");
const model_driven_app_page_1 = require("./model-driven-app.page");
const types_1 = require("../types");
const app_helpers_1 = require("../utils/app-helpers");
class PowerAppsPage {
    constructor(page) {
        this.page = page;
        this.locators = new power_apps_locators_1.PowerAppsPageLocators(page);
        this.canvas = new canvas_app_page_1.CanvasAppPage(page);
        this.modelDriven = new model_driven_app_page_1.ModelDrivenAppPage(page);
    }
    // ========================================
    // Navigation Methods
    // ========================================
     ...
     */
    async navigateToApps(options = {}) {
        const { url = '', waitForLoad = true, timeout = 60000 } = options;
        const appsUrl = url + types_1.EndPointURL.apps;
     ...
     */
    async navigateToSolutions(options = {}) {
        const { url = '', waitForLoad = true, timeout = 60000 } = options;
        const solutionsUrl = url + types_1.EndPointURL.solutions;
     ...
     * @param options - Navigation options
     */
    async navigateToAppsViaMenu(options = {}) {
        const { url = '', waitForLoad = true, timeout = 60000 } = options;
     ...
     * @param options - Navigation options
     */
    async navigateToSolutionsViaMenu(options = {}) {
        const { url = '', waitForLoad = true, timeout = 60000 } = options;
        // First navigate to home to ensure menu is available
        await this.navigateToHome({ url, waitForLoad: true, timeout });
     ...
     */
    async waitForHomePageLoad(timeout = 60000) {
        ...
        await (0, app_helpers_1.waitForSpinnerToDisappear)(this.page);
    }
    /**
     * Wait for Apps page to fully load
     * @param timeout - Timeout in milliseconds
     */
    async waitForAppsPageLoad(timeout = 60000) {
     ...
     * @param timeout - Timeout in milliseconds
     */
    async waitForSolutionsPageLoad(timeout = 60000) {
     ...
     */
    async findSolution(solutionName, options = {}) {
        const { timeout = 30000 } = options;
     ...
     */
    async createApp(appType, appName) {
        // Ensure we're on apps page
        if (!(await this.isAppsPage())) {
            await this.navigateToApps();
        }
     ...
     */
    async deleteApp(appType, appName) {
        // Find the app
     ...
     */
    async openApp(appName, appType, mode = 'edit') {
        const appLocator = await this.findApp(appName);
        if (mode === 'edit') {
            ...
            if (appType === types_1.AppType.ModelDriven) {
     ...
     */
    async openModelDrivenAppInNewWindow(appName) {
        const appLocator = await this.findApp(appName);
        const [newPage] = await Promise.all([
            this.page.context().waitForEvent('page'),
            appLocator.click(),
    ...
    async verifyHomePageLoaded() {
        await (0, test_1.expect)(this.locators.root).toBeVisible();
        await (0, test_1.expect)(this.locators.pageHeader).toBeVisible();
        await (0, test_1.expect)(this.locators.mainNavigation).toBeVisible();
    }
    /**
     * Verify apps page loaded successfully
     */
    async verifyAppsPageLoaded() {
        await (0, test_1.expect)(this.locators.appsPageMainContainer).toBeVisible();
        await (0, test_1.expect)(this.locators.sidebar).toBeVisible();
        await (0, test_1.expect)(this.locators.commandBar).toBeVisible();
    }
    /**
     * Verify solutions page loaded successfully
     */
    async verifySolutionsPageLoaded() {
        await (0, test_1.expect)(this.locators.solutionsSidebar).toBeVisible();
        await (0, test_1.expect)(this.locators.solutionsCommandBar).toBeVisible();
        await (0, test_1.expect)(this.locators.solutionsListContainer).toBeVisible();
    }
    /**
    ...
    async verifyAppExists(appName) {
        const appLocator = await this.findApp(appName);
        await (0, test_1.expect)(appLocator).toBeVisible();
    }
    /**
     * Verify solution exists in the list
     * @param solutionName - Name of the solution
     */
    async verifySolutionExists(solutionName) {
        const solutionLocator = await this.findSolution(solutionName);
        await (0, test_1.expect)(solutionLocator).toBeVisible();
    }
}
exports.PowerAppsPage = PowerAppsPage;

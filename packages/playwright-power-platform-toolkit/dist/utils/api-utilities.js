...
exports.extractTokenFromStorage = extractTokenFromStorage;
exports.createAuthenticatedApiContext = createAuthenticatedApiContext;
exports.buildBapApiEndpoint = buildBapApiEndpoint;
exports.createBapApiHeaders = createBapApiHeaders;
exports.buildResourceQueryEndpoint = buildResourceQueryEndpoint;
exports.createQueryPayload = createQueryPayload;
exports.extractRequestId = extractRequestId;
exports.validateApiResponse = validateApiResponse;
exports.pollOperationStatus = pollOperationStatus;
 ...
 * ```
 */
function extractTokenFromStorage(apiUrl) {
    const token = config_1.ConfigHelper.getAuthToken(undefined, apiUrl);
    if (!token) {
        throw new Error(`No authentication token found. Please ensure you have authenticated via: npm run auth:headful${apiUrl ? `\nNote: Token must have correct audience for API: ${apiUrl}` : ''}`);
    }
    return token;
}
/**
 ...
 * ```
 */
async function createAuthenticatedApiContext(token, baseUrl) {
 ...
 * ```
 */
function buildBapApiEndpoint(pathQuery) {
    const baseUrl = config_1.ConfigHelper.getBapApiUrl();
 ...
 * ```
 */
function createBapApiHeaders(pathQuery, token) {
    return {
        authorization: `Bearer ${token}`,
        'x-ms-user-agent': 'Playwright-Test/1.0.0',
...
function buildResourceQueryEndpoint() {
    const baseUrl = config_1.ConfigHelper.getBaseUrl();
 ...
 * ```
 */
function createQueryPayload(options) {
    return {
        query: {
            skip: options?.skip ?? 0,
            top: options?.top ?? 50,
 ...
 * ```
 */
async function pollOperationStatus(operationUrl, token, maxAttempts = 30, intervalMs = 2000) {
